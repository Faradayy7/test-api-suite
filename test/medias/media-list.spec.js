...
test.describe("GET-media", () => {
  ...
  test("TC-MEDIA-003 id", async ({ request }) => {
    const api = new ApiClient(request);
    const res = await api.get("/api/media", { id: "643e988a5de1a018e611a894" });
    expect(res.status).toBe(200);
    // Validar que el campo data es un número
    expect(typeof res.data?.data).toBe("objet");
    // Validar que el ID del primer elemento coincide con el solicitado
    const items = res.data?.data ?? [];
    if (items.length > 0) {
      expect(items[0].id).toBe("643e988a5de1a018e611a894");
    }
    console.log("Evidencia: ID obtenido", items[0]?.id);
  });
  ...
  test("TC-MEDIA-004 limit respeta máximo y count=true retorna conteo", async ({
    ...
    const api = new ApiClient(request);
    const res = await api.get("/api/media", { limit: 100, count: true });
    expect(res.status).toBe(200);
  ...
  test("Búsqueda por query y title-rule=contains (default)", async ({
    request,
  }) => {
    const api = new ApiClient(request);
    const res = await api.get("/api/media", { query: "video", limit: 10 });
    expect(res.status).toBe(200);
  });

  test("TC-MEDIA-005 Orden ascendente/descendente por date_created", async ({
    request,
  }) => {
    const api = new ApiClient(request);
  ...
  test("TC-MEDIA-006 Filtros combinados: type=audio + published=true", async ({
    request,
  }) => {
    const api = new ApiClient(request);
    const res = await api.get("/api/media", {
  ...
  test("TC-MEDIA-007 Filtro por type=video", async ({ request }) => {
    const api = new ApiClient(request);
  ...
  test("TC-MEDIA-008 Búsqueda por query (subtexto en title)", async ({
    request,
  }) => {
    const api = new ApiClient(request);
    const res = await api.get("/api/media", { query: "video", limit: 10 });
  ...
  test("Regla title-rule=starts_with", async ({ request }) => {
    const palabra = "Intro";
  ...
  test("Rango de fechas created_after / created_before", async ({
    request,
  }) => {
    const api = new ApiClient(request);
    const res = await api.get("/api/media", {
  ...
  test("Orden por -date_created (desc)", async ({ request }) => {
    const api = new ApiClient(request);
    const res = await api.get("/api/media", {
      sort: "-date_created",
      limit: 10,
